/* 「导出分享」取证 · 第二步：抠出驱动报告，打印判定 + 非零退出码。
 * 运行： node verify/export-share-parse.js [dump 路径]   （默认 verify/export-share-dump.html）
 */
'use strict';
const fs = require('fs');
const path = require('path');

const p = process.argv[2] || path.join(__dirname, 'export-share-dump.html');
