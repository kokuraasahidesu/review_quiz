/* 「AI 入口」取证 · 第二步：从 dump 里抠出驱动报告，打印判定并给出非零退出码。
 * 运行： node verify/ai-entry-parse.js [dump 路径]   （默认 verify/ai-entry-dump.html）
 */
'use strict';
const fs = require('fs');
const path = require('path');

const p = process.argv[2] || path.join(__dirname, 'ai-entry-dump.html');
