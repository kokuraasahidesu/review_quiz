...
(function (root, factory) {
  ...
  const CSS = [
    ...
    /* 判分之后标"对/错"（用户要求：**你选错的那个标红、正确的标绿**）。
     * ⚠ 这两条必须排在 `aria-pressed` 那条**后面**：同特异度时后者会盖掉前者，
     *   而"选中的错项"同时满足两个条件 —— 排后面才轮得到红色接管。 */
    '.av-opt[data-mark="ok"]{border-color:var(--av-ok);background:#e9f7ee;box-shadow:inset 3px 0 0 var(--av-ok)}',
    '.av-opt[data-mark="ok"] .mk{display:inline-flex;background:var(--av-ok)}',
    '.av-opt[data-mark="bad"]{border-color:var(--av-bad);background:#fdf1f0;box-shadow:inset 3px 0 0 var(--av-bad)}',
    '.av-opt[data-mark="bad"] .mk{display:inline-flex;background:var(--av-bad)}',
    '.av-opt[data-mark="ok"] .lb{border-color:var(--av-ok);color:var(--av-ok)}',
    '.av-opt[data-mark="bad"] .lb{border-color:var(--av-bad);color:var(--av-bad)}',
    ...
    /* 窄屏**答题中**（用户本轮要求："整个题号功能移到答案解析下面"）：题干 → 答案解析 → 题号跳转。
     * 用 flex order 摆，不动 DOM —— 宽屏那套 grid-area 布位与窄屏的阅读顺序各自都对。 */
    '.av-root:not(.av-wide):not(.av-finished) .av-body{display:flex;flex-direction:column}',
    '.av-root:not(.av-wide):not(.av-finished) .av-main{order:1}',
    '.av-root:not(.av-wide):not(.av-finished) .av-side{order:2}',
     ...
     *   · **答题中**（:not(.av-finished)）：题号跳转搬到**答案解析下面**（用户本轮要求：
     *     "整个题号功能移到答案解析下面"），右栏顺序 = 答案解析 → 题号跳转。 */
    'grid-template-areas:"q n" "q a"}',
    '.av-root.av-wide:not(.av-finished) .av-body{grid-template-areas:"q a" "q n"}',
    ...
    /* 题号索引：**说明与题号同一排**（用户要求"说明不要单独列出来，和功能放一排"）——
     * 所以整块是个 flex 行：左边一句短说明，右边就是可点的题号格。 */
    '.av-nav{display:flex;flex-wrap:wrap;align-items:flex-start;gap:8px 10px;border:1px solid var(--av-line);',
    'border-radius:10px;padding:8px 10px;background:#fbfcfd;margin:0 0 12px}',
    '.av-nav .h{flex:0 0 auto;display:flex;align-items:center;gap:8px;min-height:48px;font-size:13px;color:var(--av-dim);white-space:nowrap}',
    '.av-grid{flex:1 1 240px;display:grid;grid-template-columns:repeat(auto-fill,minmax(48px,1fr));gap:8px}',
    /* 大卷不要把题号网格铺满一屏：超过三行就内部滚动（题号区现在在解析下面，折叠会把说明一起藏掉） */
    '.av-nav.av-nav-many .av-grid{max-height:180px;overflow:auto}',
    ...
    /* 题型切换时的醒目悬浮提示（用户要求"每次切换题型时要悬浮窗醒目标注"）：
     * 固定居中、黑底白字、约 1.8 秒后自己淡出；`pointer-events:none` → 不挡任何点击。 */
    '.av-typeflash{position:fixed;left:50%;top:14px;transform:translateX(-50%);z-index:60;pointer-events:none;',
    'padding:10px 20px;border-radius:999px;background:rgba(27,27,31,.93);color:#fff;font:600 17px/1.3 inherit;',
    'box-shadow:0 10px 28px rgba(0,0,0,.3);animation-name:av-typeflash;animation-duration:1.8s;',
    'animation-timing-function:ease-out;animation-fill-mode:forwards}',
    '@keyframes av-typeflash{0%{opacity:0;transform:translate(-50%,-14px) scale(.92)}',
    '12%{opacity:1;transform:translate(-50%,0) scale(1.08)}22%{transform:translate(-50%,0) scale(1)}',
    '78%{opacity:1}100%{opacity:0;transform:translate(-50%,-10px) scale(1)}}',
    '@media (prefers-reduced-motion:reduce){.av-typeflash{animation:none}}',
  ...
  function mount(opts) {
    ...
    function optionButton(m, value, label, disabled) {
      ...
      /* 判分之后给选项标对错（用户要求："答错之后你选的错误选项标红，正确选项标注正确的颜色"）。
       * 规则：正确答案那一项 → 绿；**你选了但它不是答案** → 红；没选且不是答案 → 不动。
       * ⚠ 只在"已提交 **且** 允许揭示"时才标 —— 否则等于把答案提前画在选项上（那是泄题）。 */
      let mark = '';
      if (m.submitted && m.revealed && q.kind !== 'text') {
        let isCorrect = null;
        if (q.kind === 'judge') {
          if (m.judgeTrue != null) isCorrect = ((value === 'true') === m.judgeTrue);
        } else {
          /* ⚠ 答案从 **`m.answerLetters`**（模型顶层，只在允许揭示时给值）读 ——
           *   第一版写成 `q.answerLetters`（题目子对象里没有这个字段）→ 正确选项永远标不上绿。 */
          isCorrect = (m.answerLetters || []).indexOf(String(value)) >= 0;
        }
        if (isCorrect === true) mark = 'ok';
        else if (isCorrect === false && pressed) mark = 'bad';
      }
      ...
      /* 角标：选中显示 ✓；判分标错时换成 ✗（颜色之外还有字形区分，色弱也能认） */
      const mk = el(doc, 'span', 'mk', mark === 'bad' ? '✗' : '✓');
    ...
    function paintReveal(m, box) {
      ...
      p.appendChild(head);
      if (m.revealed) {
        /* ⚠ 这里原来还有一句「答错了：看完正确答案自己点「下一题」」——
         *   用户明确要求**删掉**：答错时选项已经标红/标绿、下面也写着正确答案，再叮嘱一遍是噪音。 */
    ...
    /* ---------- 题号索引（点哪跳哪 + 状态实时） ---------- */
    /* 用户要求：「题号跳转功能说明不要单独列出来，和功能放一排」。
     * 所以**不再**有一行独立的说明（原来那行写着"题号（点一下跳过去）"），也不再用折叠 `<details>`
     * （折叠起来会把唯一的说明一起藏掉）：现在是一行 —— 左边短说明「题号 · 点一下跳转」，
     * 右边紧挨着就是题号格；题多时网格内部滚动（`.av-nav-many`），不占满屏。 */
    function paintNav(m, box) {
      const nav = el(doc, 'div', 'av-nav');
      const head = el(doc, 'div', 'h');
      head.appendChild(el(doc, 'span', null, '题号 · 点一下跳转'));
      ...
      if (cells.length > 30) nav.classList.add('av-nav-many');   // 大卷：网格内部滚（说明仍在同一排）
    ...
    /* ---------- 题型切换的醒目悬浮提示（用户要求"每次切换题型时要悬浮窗醒目标注"） ----------
     * 只在**题型真的变了**时弹（单选↔多选↔判断↔简答），同一题型连着答不打扰；
     * 固定居中、黑底白字，1.8 秒后自己淡出（`pointer-events:none`，不挡任何点击）。
     * ⚠ 判据落在**题号 + 题型**上：repaint（判分/勾选/刷新）不该重新弹一次。 */
    let flashType = null, flashKey = null, flashTimer = null, flashEl = null;
    function showTypeFlash(label) {
      if (flashTimer !== null) { clearTimeout(flashTimer); flashTimer = null; }
      /* ⚠ 用**缓存引用**摘掉上一块，不用 `rootEl.querySelector`：Node 侧的极简 DOM 桩没实现它
       *   （测试里就是靠这个桩跑整条渲染链的，别让一个新特性把桩打穿）。 */
      if (flashEl && flashEl.parentNode) flashEl.parentNode.removeChild(flashEl);
      const f = el(doc, 'div', 'av-typeflash', '切换到「' + label + '」');
      f.setAttribute('data-av', 'typeflash');
      ...
      flashTimer = setTimeout(function () {
        ...
        if (flashEl === f) flashEl = null;
      }, 1800);
    }
    function maybeFlashType(m) {
      if (!m || !m.question) return;
      const key = m.index + '|' + m.question.type;
      if (key === flashKey) return;                     // 同一题的 repaint：不弹
      const changed = flashType !== null && flashType !== m.question.type;
      flashKey = key;
      flashType = m.question.type;
      if (changed && !m.finished) showTypeFlash(m.question.typeLabel || m.question.type);
    }
    ...
    function paint() {
      ...
      maybeFlashType(m);                             // 换了题型就弹一下悬浮提示（用户要求）
      refreshActions();
    ...
    return {
      ...
      destroy: function () {
        ...
        if (flashTimer !== null) { clearTimeout(flashTimer); flashTimer = null; }   // 题型提示的定时器同理
